...
def kinematic_sim():
    ...
    ax['A'].plot(*sys.hook_pos(), label='hook path'); ax['A'].set_aspect('equal', adjustable='box')
    arm, = ax['A'].plot(*sys.arm_pos, 'ro-')
    xvel, yvel = sys.hook_vel()
    ax['C'].plot(np.rad2deg(sys.theta[:, 1]), xvel, label='hook x-velocity')
    ax['C'].plot(np.rad2deg(sys.theta[:, 1]), yvel, label='hook y-velocity')
