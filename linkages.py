...
class LinkageSystem():
    def __init__(self, 
        ...
        # initialize theta, w, alpha matrices
        ...
        self.xy_position()              # solve for x,y relative to body fixed origin (origin addes as last column)
        self.xy_velocity()
        self.xy_acceleration()          # integrate velocity bc I don't want to do the kinematics
    ...
    def xy_velocity(self):
        neg_sin_pos_cos = np.stack([-1*np.sin(self.theta), np.cos(self.theta)], axis=2)
        dxdy = np.einsum('i,ji,jik->jik', self.L, self.w, neg_sin_pos_cos)

        self.joint_vel = np.zeros(dxdy.shape)

        for link, map in self.mapping.items():
            self.joint_vel[:,link,:] = np.sum(dxdy[:,map,:], axis=1) + self.origin

        self.link_velocities = np.append(self.joint_vel, (np.ones((self.joint_vel.shape[0], 1, self.joint_vel.shape[2])) * self.origin), axis=1)

    def xy_acceleration(self):
        self.link_accelerations = np.diff(self.link_velocities, axis=0)
...
class Klann(LinkageSystem):
    ...
    def angular_vel(self):
        ...
        self.w[:, 8] = self.w[:, 7]               # w7 = w8 (colinear link7 & link8)
    ...
    def fivebar_vel(self):
        ...
        guesses = [0, 0] # initial guesses for when w[:, 1] = 1
